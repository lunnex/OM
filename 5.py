...
def printGeneration(arr):
    ...
    print(np.average(xs), np.average(x1s), np.average(ys), np.average(chances))
    print("Особь с наибольшим значением функциии полезности")
    print(arr[numOfMax].x, arr[numOfMax].x1, arr[numOfMax].y, arr[numOfMax].chance)
...
for i in range(0, sizeOfGeneration):
    ...
    x1 = random.randrange(a * 1000, b * 1000) / 1000
    y = func(x, x1)
    inhabitants.insert(i, Inhabitant(x, x1, y))
...
def OnePointCrossoverWithCopy():
    ...
    b1 = float2bin(pair[0].x1)
    b1cut1 = b1[0:cut]
    b1cut2 = b1[cut:64]

    b2 = float2bin(pair[1].x1)
    b2cut1 = b2[0:cut]
    b2cut2 = b2[cut:64]
    ...
    if (random.randrange(0, 1000) > mutationThreshold):
        rndR1 = random.randrange(30, len(ch1x))
        ch1x = setMutation(ch1x, rndR1)
        ch1x1 = setMutation(ch1x1, rndR1)

        rndR2 = random.randrange(30, len(ch2x))
        ch2x = setMutation(ch2x, rndR2)
        ch2x1 = setMutation(ch2x1, rndR2)

    if (bin2float(ch1x) >= a and bin2float(ch1x) <= b and bin2float(ch1x1) >= a and bin2float(ch1x1) <= b):
        inhabitant = Inhabitant(bin2float(ch1x), bin2float(ch1x1), func(bin2float(ch1x), bin2float(ch1x1)))
        newGeneration.append(inhabitant)
    if (bin2float(ch2x) >= a and bin2float(ch2x) <= b and bin2float(ch2x1) >= a and bin2float(ch2x1) <= b):
        inhabitant = Inhabitant(bin2float(ch2x), bin2float(ch2x1), func(bin2float(ch2x), bin2float(ch2x1)))
        newGeneration.append(inhabitant)
...
def StandartCrossover():
    ...
    ch1x = s1 + p11 + p22 + m11 + m22
    ch2x = s2 + p12 + p21 + m12 + m21
    ...
    ch1x1 = s1 + p11 + p22 + m11 + m22
    ch2x1 = s2 + p12 + p21 + m12 + m21
...
def TwoPointCrossover():
    ...
    ch1x = b1cut1 + b2cut2 + b1cut3
    ch2x = b2cut1 + b1cut2 + b2cut3
    ...
    ch1x1 = b1cut1 + b2cut2 + b1cut3
    ch2x1 = b2cut1 + b1cut2 + b2cut3
    ...
    try:
        if (bin2float(ch1x) >= a and bin2float(ch1x) <= b and bin2float(ch1x1) >= a and bin2float(ch1x1)):
            inhabitant = Inhabitant(bin2float(ch1x), bin2float(ch1x1), func(bin2float(ch1x), bin2float(ch1x1)))
            newGeneration.append(inhabitant)
        if (bin2float(ch2x) >= a and bin2float(ch2x) <= b and bin2float(ch2x1) >= a and bin2float(ch2x1) <= b):
            inhabitant = Inhabitant(bin2float(ch2x), bin2float(ch2x1), func(bin2float(ch2x), bin2float(ch2x1)))
